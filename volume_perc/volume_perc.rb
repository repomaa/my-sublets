...
helper do
    def gen_data
        unless self.percent
            "%s%s" % [ self.icons[self.mixer.state], self.mixer.icon ]
        else
            data = self.icons[self.mixer.state]
            if self.mixer.volume.first < 10
                data += "  "
            elsif self.mixer.volume.first < 100
                data += " "
            end
            data += "#{self.mixer.volume.first}%"
        end
    end
end
# Hooks
